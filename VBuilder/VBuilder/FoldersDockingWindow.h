...
public:
	...
	BEGIN_MSG_MAP(thisClass)
		...
		//COMMAND_ID_HANDLER(ID_RELOAD, OnReload)
		//COMMAND_ID_HANDLER(ID_DELETE, OnDelete)



		NOTIFY_CODE_HANDLER(TVN_ITEMEXPANDING, OnTvnItemexpandingTree)
		NOTIFY_CODE_HANDLER(TVN_SELCHANGED, OnTvnSelchangedTree)
	...
	LRESULT OnTvnSelchangedTree(int /*idCtrl*/, LPNMHDR pNMHDR, BOOL& /*bHandled*/)
	{
		...
		// TODO: 在此添加控件通知处理程序代码
		GetParent().GetParent().SendMessage(TVN_SELCHANGED,(WPARAM)pNMHDR);
		return 0;
	}

	LRESULT OnTvnItemexpandingTree(int /*idCtrl*/, LPNMHDR pNMHDR, BOOL& /*bHandled*/)
	{
		LPNMTREEVIEW pNMTreeView = reinterpret_cast<LPNMTREEVIEW>(pNMHDR);
		// TODO: 在此添加控件通知处理程序代码
		GetParent().GetParent().SendMessage(TVN_ITEMEXPANDING,(WPARAM)pNMHDR);
		return 0;
	}
...
//		FillTree(m_tree);
	...
	LRESULT OnSize(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		if(wParam != SIZE_MINIMIZED )
		{
			RECT rc;
			GetClientRect(&rc);
			::SetWindowPos(m_tree.m_hWnd, m_hWnd, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top ,SWP_NOZORDER | SWP_NOACTIVATE);
		}
		bHandled = FALSE;
		return 1;
	}
	...
	LRESULT OnNMDblclkTree(int /*idCtrl*/, LPNMHDR pNMHDR, BOOL& /*bHandled*/)
	{
		// TODO: 在此添加控件通知处理程序代码
	...
	LRESULT OnNMRclickTree(int /*idCtrl*/, LPNMHDR pNMHDR, BOOL& /*bHandled*/)
	{
		...
		popmenu.AppendMenu(0,ID_ADD_INST,_T("add"));
		popmenu.AppendMenu(0,ID_RELOAD,_T("reload"));		
		popmenu.AppendMenu(0,ID_DELETE,_T("delete"));
