...
CAtlRegExp<CharTraits> atlRegExp;
	...
	*/
	HRESULT SubstituteVars(CStringA &original,CAtlREMatchContext<CharTraits> &atlRematchContext){
		CAtlREMatchContext<CharTraits>::MatchGroup tempGroup;
		HRESULT hr=S_OK;
		char *buffer;
		size_t bufSize = (atlRematchContext.m_uNumGroups/10)+2;
		buffer = new char[bufSize];
		CStringA temp,variableid;
		UINT i,y;
	...
	*/
	CStringA getFoundat(int pos,CAtlREMatchContext<CharTraits> &atlRematchContext){
		CStringA temp;
		temp="";
		int y;
...
public:
	...
	*/
	HRESULT s(CStringA &baseStr,CStringA oldexp,CStringA newexp,const bool &global=false,const bool &usingvars=false,const bool &casesensitive=true){

		HRESULT hr;

		CAtlREMatchContext<CharTraits> atlRematchContext;
		CStringA substitutedexpr,strRightSide;
	...
	*/	
	int FindAllInstances(CAtlArray<CStringA> &allinstances,CStringA basestring,CStringA oldexp){
		int count=0;
	...
	*/
	CStringA GetInstanceNum(int pos,CStringA basestring,CStringA oldexp){
