...
class CCategoryProperty : public CPropertyItem
{
public:
   CSimpleArray<IProperty*> m_arrItems;
   bool m_fExpanded;

   CCategoryProperty(CString sName, LPARAM lParam):CPropertyItem(sName,lParam), m_fExpanded(true)
   {
   ...
   void DrawName(PROPERTYDRAWINFO& di,UINT iCol=0)
   {
      CDCHandle dc(di.hDC);
      ...
      if( (di.dwExtStyle & PLS_EX_XPLOOK) != 0 ) {
         ...
         CPropertyItem::DrawName(di);
         if( (di.state & ODS_SELECTED) != 0 ) {
            RECT rcFocus = di.rcItem;
            ::InflateRect(&rcFocus, -1, -1);
            SIZE size = { 0 };
            dc.GetTextExtent(m_sName, m_sName.GetLength(), &size);
   ...
   void DrawValue(PROPERTYDRAWINFO& di,UINT iCol=0) 
   { 
      if( (di.dwExtStyle & PLS_EX_XPLOOK) != 0 ) {
...
public:
   ...
   void SetColumNum(UINT n) 
   {
		...
		for (int i=1;i<m_iColumNum;i++) {
			m_iMiddle[i]  = (rc.right - rc.left) * i/ m_iColumNum;
		}
   }
...
//      ATLASSERT(prop->GetKind()!=PROPKIND_CATEGORY);
   ...
   int GetColumnWidth(int idx) const
   {
      return m_iMiddle[idx];
   }

   void SetColumnWidth(int idx,int iWidth)
   {
      ATLASSERT(::IsWindow(m_hWnd));
      m_iMiddle[idx] = iWidth;
      m_bColumnFixed = true;
      Invalidate();
   }
   ...
   void _GetInPlaceRect(int idx, RECT& rc) const
   {
	  ...
	  rc.right = rc.left + m_iMiddle[1];
      rc.left += m_iMiddle[0] + 1;
   }
   ...
   {      
	  ...
	  if( !m_bColumnFixed ) { 
		  for (int i=1;i<m_iColumNum;i++) {
			  m_iMiddle[i-1]  = (rc.right - rc.left) * i/ m_iColumNum;
		  }
	  }
	  m_iPrevious = 0;
      BOOL bDummy = FALSE;
   ...
   LRESULT OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
	  ...
	  if( (m_dwExtStyle & PLS_EX_NOCOLUMNRESIZE) == 0) {
		  for (int i=0;i<m_iColumNum-1;i++)  {
			  if (GET_X_LPARAM(lParam) == m_iMiddle[i] + iIndent ) {
					m_iMidIdex = i;
   ...
   LRESULT OnLButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
   {
      ...
      if( m_iPrevious > 0 ) {
		  for (int i=m_iPrevious;i<m_iColumNum-1;i++) {
			m_iMiddle[i] += x - m_iPrevious;
		  }
         Invalidate();
      }
      m_iPrevious = 0;
   ...
   LRESULT OnChangedProperty(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      ...
      LPCTSTR pVariant = reinterpret_cast<LPCTSTR>(wParam);
      ATLASSERT(prop && pVariant);
      if( prop == NULL || pVariant == NULL ) return 0;
	  CString sVal = pVariant;
      ...
      if( ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
         ...
         if( !prop->SetValue(sVal,0) ) ::MessageBeep((UINT)-1);
         // Let owner know
         nmh.hdr.code = PIN_ITEMCHANGED;
         ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
      }
   ...
   void DrawItem(LPDRAWITEMSTRUCT lpDIS)
   {
      ...
      rcName.right = rc.left + m_iMiddle[0];
	  rcValue.left = rc.left + m_iMiddle[0] + 1;
	  rcValue.right = rc.left + m_iMiddle[1];

      // Special handling of XP-like categories
      if( kind == PROPKIND_CATEGORY && (m_dwExtStyle & PLS_EX_XPLOOK) != 0 ) {
         rcName.right =  rc.right;
      }
      ...
      // check below...
      if( kind != PROPKIND_CATEGORY && (int) lpDIS->itemID != m_iInplaceIndex ) {
		  for (int i=0;i<m_iColumNum-1;i++) {
			  rcValue.left= rc.left + m_iMiddle[i] + 1;
			  rcValue.right = i+1 >= m_iColumNum-1 ? rc.right : rc.left + m_iMiddle[i+1];
      ...
      if( !(kind == PROPKIND_CATEGORY && (m_dwExtStyle & PLS_EX_XPLOOK) != 0) ) {
		  for (int i=0;i<m_iColumNum-1;i++) {
			  dc.MoveTo(rc.left + m_iMiddle[i], rc.top);
			  dc.LineTo(rc.left + m_iMiddle[i], rc.bottom - 1);
		  }
      }
      dc.SelectPen(hOldPen);
